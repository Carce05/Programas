...
class AddProgramaFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        //Se agrega la funcion para agregar un programa
    ...
    private fun subeAudio() {
        if (audioUtiles.getAudioGrabado()) {
            ...
            if (audioFile.exists() && audioFile.isFile && audioFile.canRead()) {
                ...
                val rutaNube =
                    "programaeApp/${Firebase.auth.currentUser?.email}/audios/${audioFile.name}"
    ...
    private fun subeImagen(rutaAudio: String) {
        ...
        if (imagenUtiles.getFotoTomada()) {
            ...
            if (imagenFile.exists() && imagenFile.isFile && imagenFile.canRead()) {
                ...
                val rutaNube =
                    "programaeApp/${Firebase.auth.currentUser?.email}/imagenes/${imagenFile.name}"
                val referencia: StorageReference = Firebase.storage.reference.child(rutaNube)
                referencia.putFile(ruta)
                    .addOnSuccessListener {
                        referencia.downloadUrl
                            .addOnSuccessListener {
                                ...
                                addPrograma(rutaAudio, rutaImagen)
                            }
                    }
                    .addOnFailureListener {
                        addPrograma(rutaAudio, "")
    ...
    private fun addPrograma(rutaAudio: String, rutaImagen: String) {
